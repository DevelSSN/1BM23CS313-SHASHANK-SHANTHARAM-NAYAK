...
class GEP(object):
    def __init__(self, grammar, bounds, len_head, iterations, pop_size, prob_crossover, num_trials):
        ...
        self.population = [Individual(self.random_genome())
    ...
    def point_mutation(self, genome):
        ...
        for i in range(len(genome)):
            ...
            if np.random.random() < rate:
                if i < self.len_head:
                    selection = self.grammar['FUNC'] if np.random.random(
                    ) < 0.5 else self.grammar['TERM']
                    bit = selection[np.random.randint(0, len(selection))]
                else:
                    bit = self.grammar['TERM'][np.random.randint(
                        0, len(self.grammar['TERM']))]
            child.append(bit)
        return ''.join(child)
    ...
    def reproduce(self, selected):
        ...
        for a, b in zip(selected[::2], selected[1::2]):
            children.append(Individual(self.point_mutation(
                self.crossover(a.genome, b.genome))))
        return children
    ...
    def random_genome(self):
        ...
        for _ in range(self.len_tail):
            s.append(self.grammar['TERM'][np.random.randint(
                0, len(self.grammar['TERM']))])
        return ''.join(s)
    ...
    def run(self):
        for gen in range(1, self.iterations + 1):
            ...
            self.population = sorted(
                (children + self.population), key=lambda x: x.fitness)[:self.pop_size]
            if gen % 50 == 0:
...
def main():
    gep = GEP(GRAMMAR, BOUNDS, LEN_HEAD, ITERATIONS,
              POP_SIZE, PROB_CROSS, NUM_TRIALS)
    gep.run()
